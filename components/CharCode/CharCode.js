export default class CharCode {
  static regTestUtf8 = /^(\\x[0-9a-f]{2})*$/i;
	static regMatchUtf8 = /\\x(?<code>[0-9a-f]{2})/ig;
  
  constructor() {}

  // 拼装unicode
  static assembleUnicode = arr => {
  ...
  utf8ToUnicode(string) {
    ...
    if(CharCode.regTestUtf8.test(string)) {
      // 编码转换
      ...
      for(let item of matches) {
        ...
        if((code & 0b1000_0000) === 0b0000_0000) {
          ...
          if(multiBytesArr.length) {
            transfers.push(CharCode.assembleUnicode(multiBytesArr));
          }
  ...
  unicodeToUtf8(unicode) {
    ...
    for(let code of unicodeArr) {
      ...
      if(code <= 0x7f) {
        // 单字节
        utf8Arr.push('\\x' + parseInt(code.toString(2).padStart(8, '0'), 2).toString(16));
      } else if(code <= 0x7ff) {
        // 2字节
